...
@section('title', 'Manage Rental Requests')
...
<div class="flex-1 flex flex-col px-4 md:px-8 py-8 w-full">
    <div class="bg-base-100 shadow-xl rounded-2xl p-8 w-full mx-auto">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-8">
            <div>
                <h1 class="text-3xl font-bold text-base-content">Manage Rental Requests</h1>
                <p class="text-base-content/60 mt-1">View and manage all rental requests from tenants</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.rental-requests.create') }}" class="btn btn-primary">
            ...
            <div class="flex-1">
                <div class="form-control">
                    <div class="input-group">
                        <input type="text" placeholder="Search rental requests..." class="input input-bordered flex-1" id="searchInput" />
                ...
                <tbody>
                    @forelse($rentalRequests as $rentalRequest)
                        <tr class="hover:bg-base-200/50 transition-colors">
                            <td>
                                <div>
                                    <div class="font-bold text-primary">RR-{{ str_pad($rentalRequest->rental_request_id, 3, '0', STR_PAD_LEFT) }}</div>
                                    <div class="text-sm text-base-content/60">ID: {{ $rentalRequest->rental_request_id }}</div>
                                </div>
                            </td>
                            <td>
                                <div>
                            ...
                            <td>
                                <div>
                                    <div class="font-medium text-base-content">{{ $rentalRequest->duration }} {{ $rentalRequest->duration_type }}</div>
                                    <div class="text-sm text-base-content/60">{{ $rentalRequest->start_date->format('M d, Y') }} - {{ $rentalRequest->end_date->format('M d, Y') }}</div>
                                </div>
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="text-sm text-base-content/60">
                                    {{ $rentalRequest->created_at ? $rentalRequest->created_at->format('M d, Y') : 'N/A' }}
                                </div>
                            </td>
                            <td>
                                <div class="flex justify-center gap-1">
                                    <a href="{{ route('admin.rental-requests.edit', $rentalRequest) }}" 
                        ...
                        <tr>
                            <td colspan="7" class="text-center py-12">
                                <div class="flex flex-col items-center gap-4">
                                    ...
                                    <div>
                                        <h3 class="text-lg font-semibold text-base-content">No rental requests found</h3>
                                        <p class="text-base-content/60">Get started by creating your first rental request.</p>
                                    </div>
                                    <a href="{{ route('admin.rental-requests.create') }}" class="btn btn-primary">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                                        </svg>
                                        Add First Rental Request
                                    </a>
                                </div>
                            </td>
                        </tr>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mt-6 pt-6 border-t border-base-300">
            <!-- Results Info -->
            <div class="text-sm text-base-content/60">
                Showing {{ $rentalRequests->firstItem() ?? 0 }} to {{ $rentalRequests->lastItem() ?? 0 }} of {{ $rentalRequests->total() }} results
            </div>
            ...
            @if($rentalRequests->hasPages())
                <div class="join">
                    ...
                    @else
                        <a href="{{ $rentalRequests->previousPageUrl() }}" class="join-item btn btn-sm">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                            </svg>
                        </a>
                    ...
                    {{-- Page Numbers --}}
                    @foreach ($rentalRequests->getUrlRange(1, $rentalRequests->lastPage()) as $page => $url)
                        @if ($page == $rentalRequests->currentPage())
                    ...
                    {{-- Next Page --}}
                    @if ($rentalRequests->hasMorePages())
                        <a href="{{ $rentalRequests->nextPageUrl() }}" class="join-item btn btn-sm">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div id="deleteModal" class="modal">
    <div class="modal-box">
        <h3 class="font-bold text-lg">Delete Rental Request</h3>
        <p class="py-4">Are you sure you want to delete the rental request for <span id="deleteTenantName" class="font-semibold"></span>? This action cannot be undone.</p>
...
function openDeleteModal(requestId, tenantName) {
    ...
    document.getElementById('deleteForm').action = `/admin/rental-requests/${requestId}`;
