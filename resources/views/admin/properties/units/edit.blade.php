...
@section('title', 'Edit Unit for ' . $property->name)
...
<div class="flex-1 flex flex-col items-center justify-center px-4 md:px-8 py-8 w-full">
    <div class="bg-base-100 shadow-xl rounded-2xl p-8 w-full max-w-4xl mx-auto">
        <h1 class="text-2xl font-bold mb-6 text-center">Update Unit</h1>
        ...
        <form action="{{ route('properties.units.update', [$property->property_id, $unit->unit_id]) }}" method="POST" class="space-y-6" id="unitForm">
            @csrf
            @method('PUT')
                ...
                <div class="form-control" id="customUnitTypeField" style="display: none;">
                    ...
                    <div class="relative">
                        <input type="text" name="custom_type" id="custom_type" placeholder="Enter custom unit type" 
                               ...
                               value="{{ old('custom_type', (!in_array($unit->type, ['studio', 'single_story_house', 'room', 'small_unit_house', 'kitchen', 'event_hall', 'restaurant']) ? $unit->type : '')) }}" />
                        <!-- Icon -->
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <svg class="w-5 h-5 text-base-content/40" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/>
...
<script>
    ...
    function saveParameters() {
        ...
        // Store the parameters for later use when unit is updated
    ...
    function populateExistingParameters() {
        ...
        fetch(`/admin/properties/{{ $property->property_id }}/units/{{ $unit->unit_id }}/parameters`)
            .then(response => response.json())
            .then(data => {
    ...
    function toggleCustomUnitType() {
        ...
        if (unitTypeSelect.value === 'custom') {
            ...
            customTypeInput.value = ''; // Clear value if not required
        }
    }
